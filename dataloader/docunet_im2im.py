...
class Im2ImDocunet(object):
    ...
    def __init__(self, args, split=TRAIN):
        ...
        super(Im2ImDocunet, self).__init__()

        self.args = args
        self.split = split

    def preprocess_image(self, image_path, is_label=False):
        """
        Load and preprocess the input image and target vector field

        Args:
            image_path (str): disk image path
            is_label (bool): if we are preprocessing the input image or the label
        ...
        image = tf.image.resize(image, self.args.resize) if is_label else image
        image = image / 255.0
    ...
    def preprocess(self, image_path, label_path=None):
        ...
        label = self.preprocess_image(label_path, is_label=True) if label_path is not None else tf.convert_to_tensor([1])
    ...
    def load_dataset(self):
        ...
        labels_full_path = [os.path.join(labels_path, '_'.join(image_name.split('_')[:-1]) + self.LABEL_EXT) for image_name in images_name if image_name.endswith(self.DEFORMED_EXT)]
